...
from framework.learning.supervised.surrogate import *
...
class LifNodes(nn.Module):
    def __init__(self, tau=2.0, vth=1.0, reset=0.0, surrogate=rect_fn):
        super().__init__()
        self.tau = tau
        self.vth = vth
        self.reset = reset
    ...
    def forward(self, x):
        ...
        for i in range(x.shape[1]):
            u = u / self.tau + x[:, i]
            s = self.surrogate(u - self.vth)
            u = u * (1 - s) + self.reset * s
            out[:, i] = s
        return out