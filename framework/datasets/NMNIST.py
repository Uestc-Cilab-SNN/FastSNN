...
class NMNIST(SNNDataset):
    """N-MNIST数据集初始化。如果没有预处理后的.npy文件，需要手动调用一次preprocessing函数进行预处理。
        Initial an N-MNIST dataset. Need to call the function preprocessing() first if there is no *.npy file generated by preprocessing().
        Args:
    ...
    def __init__(self, train, step, dt, npyPath=None, srcPath=None, savePath=None):
        super(NMNIST, self).__init__(srcPath)
        self.step = step
        self.path = npyPath
        ...
        self.eventflow = self.read_data()
        self.label = self.get_label()

    def read_data(self):
        if self.train:
            srcPath = self.srcPath + '/Train'
            savePath = self.savePath + '/Train'
        else:
            srcPath = self.srcPath + '/Test'
            savePath = self.savePath + '/Test'

        eventflow = None
        if self.path is not None:
            if self.train:
                npyPath = self.path + '/Train'
            else:
                npyPath = self.path + '/Test'
            eventflow = np.load(npyPath + "/data.npy")
        else:
            self.preprocessing(src_path=srcPath, save_path=savePath)
            if self.train:
                eventflow = np.load(savePath + "/data.npy")
        return eventflow

    def get_label(self):
        if self.train:
            return np.load(self.savePath + '/Train' + "/label.npy")
        else:
            return np.load(self.savePath + '/Test' + "/label.npy")
    ...
    def preprocessing(self, src_path, save_path=None):
        ...
        if save_path is not None:
            ...
            np.save(save_path + '/' + field + "/data.npy", self.eventflow)
            np.save(save_path + '/' + field + "/label.npy", self.label)
