...
class NMNIST(Dataset):
    ...
    def __init__(self, train, step, dt, npyPath=None, srcPath=None, savePath=None):
        ...
        if train == True:
            ...
            srcPath = srcPath + '/Train'
            savePath = savePath + '/Train'
        else:
            self.len = 10000
        ...
        if npyPath is not None:
            if train==True:
                npyPath = npyPath + '/Train'
            else:
                npyPath = npyPath + '/Test'
            self.eventflow = np.load(npyPath + "/data.npy")
            self.label = np.load(npyPath + "/label.npy")
        else:
            self.preprocessing(src_path=srcPath, save_path=savePath)
            if train == True:
                self.eventflow = np.load(savePath + "/data.npy")
                self.label = np.load(savePath + "/label.npy")
